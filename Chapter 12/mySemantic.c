...
bool isLValueExpLegalWithResult(
    myLValueExp lValueExp, myTranslationAndType* resultPtr)
{
    *resultPtr = MySemantic_LValueExp(lValueExp);
    return (*resultPtr) != SEMANTIC_ERROR;
}
...
{
    myTranslationAndType dummy;
    return isLValueExpLegalWithResult(lValueExp, &dummy);
}
 ...
 *              analysis on SimpleVar, RecordField and ArraySubscript. Intermediate
 *              generated by LValue translator is always of Mem(addr-reg) format.
 *
    ...
    myLValueExp lValueExp, IR_myExp currentSubscriptResult)
{
    ...
    assert (result->translation->u.eseq.exp->kind == IR_Temp);
    result->translation->u.eseq.exp =
        IR_makeMem(result->translation->u.eseq.exp);
    return result;
}
...
bool isAssignmentOperandsTypeMatchOrNil(
    myType lValueExpType, myType expResultType);
...
//  STATUS:
//      Tested.
myTranslationAndType MySemantic_AssignmentExp(myAssignmentExp assignmentExp)
{
    ...
    bool isRightOperandLegal =
        isExpLegalWithResult(assignmentExp->exp, &rightResult);
    ...
    {
        IR_myExp assignmentTrans = Trans_assignment(
            leftResult->translation, rightResult->translation);
        return make_MyTranslationAndType(assignmentTrans, makeType_NoReturn());
    }
    else
    {
...
bool isAssignmentOperandsTypeMatchOrNil(myType lValueExpType, myType expResultType)
{
    myType leftOperandType = getActualType(lValueExpType);
    myType rightOperandType = getActualType(expResultType);
