...
class CFGraphTest : public CppUnit::TestFixture
{
    public:
        ...
        void testCtor_ByDefault_ConstructMaps()
        {
            ...
            RegisterNodeMap& ref = cfGraph.GetRegisterNodeMapRef();
            CPPUNIT_ASSERT_EQUAL( 4, static_cast<int>(cfGraph.GetRegisterNodeMapRef().size()) );
            CPPUNIT_ASSERT( ref[regA] != ref[regB] );
            CPPUNIT_ASSERT( ref[regA] != ref[regE] );
            CPPUNIT_ASSERT( ref[regA] != ref[regEAX] );
            CPPUNIT_ASSERT( ref[regB] != ref[regE] );
            CPPUNIT_ASSERT( ref[regB] != ref[regEAX] );
            CPPUNIT_ASSERT( ref[regE] != ref[regEAX] );
        }
