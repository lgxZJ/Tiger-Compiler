...
//  forwards
static myType getExpResultType(myTable varAndFuncEnv, myTable typeEnv, myExp exp);
static myType getTypeFromName(myTable typeEnv, mySymbol typeName);
...
static bool isExpArray(myTable varAndFuncEnv, myTable typeEnv, myExp exp)
{
    return isExpOneTypeOrIllegal(varAndFuncEnv, typeEnv, exp, TypeArray);
}

static bool isExpSameInstance(myExp lhs, myExp rhs)
{
    return lhs == rhs;
}

static bool isTwoExpsTypeMatches(
    myTable varAndFuncEnv, myTable typeEnv, myExp lhs, myExp rhs)
{
    myType leftType = getExpResultType(
            varAndFuncEnv, typeEnv, lhs);
    myType rightType = getExpResultType(
            varAndFuncEnv, typeEnv, rhs);
    ...
    myExp exp, mySymbol arrayTypeName)
{
    ...
    myType arrayElementType = getTypeFromName(typeEnv, arrayTypeName)
        ->u.typeArray->type;
    myType initialExpType = MySemantic_Exp(
        varAndFuncEnv, typeEnv, exp)->type; 
...
//      return the type of `typeName` if succeed. 
static myType getTypeFromName(myTable typeEnv, mySymbol typeName)
{
    assert (typeEnv && typeName);
    return MyEnvironment_getTypeFromName(typeEnv, typeName);
}
...
static myType getVariableType(myTable varAndFuncEnv, mySymbol variableName)
{
    ...
    return MyEnvironment_getVarOrFuncFromName(varAndFuncEnv, variableName)
                ->u.varEntry.type;
}
...
static myType getFunctionReturnType(myTable varAndFuncEnv, mySymbol functionName)
{
    ...
    return MyEnvironment_getVarOrFuncFromName(varAndFuncEnv, functionName)
                ->u.funcEntry.returnType;
}
...
static myType getActualType(myTable typeEnv, mySymbol typeName)
{
...
bool isVariableRecordType(myTable varAndFuncEnv, mySymbol variableName)
{
     ...
     return isTypeRecord(entry->u.varEntry.type);
}
...
bool isVariableArrayType(myTable varAndFuncEnv, mySymbol variableName)
{
    ...
    if (entry->kind == VarEntry)
        return isTypeArray(entry->u.varEntry.type);
    ...
    myTable varAndFuncEnv, myTable typeEnv, myLValueExp lValueExp)
{
    ...
    {
        myType elementType = arrayEntry->u.varEntry.type->u.typeArray->type;
    ...
    else
    {
        ...
        result->type = arrayEntry->u.varEntry.type->u.typeArray->type;
    }
    return result;
}
    ...
    myTable varAndFuncEnv, myTable typeEnv, myArrayCreationExp arrayCreationExp)
{
    ...
    if (isArrayTypeDefiend && isSubscriptAnIntExp && isInitialExpThisArrayElementType)
        return make_MyTranslationAndType(NULL, getTypeFromName(typeEnv, arrayTypeName));
    else
    {
    ...
    myExp initialValue, mySymbol arrayTypeName)
{
    myType arrayElementType = getTypeFromName(typeEnv, arrayTypeName)
        ->u.typeArray->type;
    myType valueType = getExpResultType(varAndFuncEnv, typeEnv, initialValue);
...
bool isTypeDefinedAsArray(myTable typeEnv, mySymbol arrayTypeName)
{
    if (isTypeDefined(typeEnv, arrayTypeName))
        return isTypeArray(getTypeFromName(typeEnv, arrayTypeName));
    return false;
}
    ...
    myNoFieldRecordCreationExp noFieldRecordCreationExp)
{
    ...
    if (isTypeDefinedAsRecord(typeEnv, recordTypeName))
        return make_MyTranslationAndType(NULL,
            getTypeFromName(typeEnv, recordTypeName));
    else
    {
        MyError_pushErrorCodeWithMsg(
            ERROR__RECORD_CREATION__TYPE_NOT_RECORD,
    ...
    myFieldRecordCreationExp fieldRecordCreationExp)
{
    ...
    if (isRecordTypeDefined && isCreationFieldsMatchThisRecordType)
        return make_MyTranslationAndType(NULL, getTypeFromName(typeEnv, recordTypeName));
    else
    {
...
bool isTypeDefinedAsRecord(myTable typeEnv, mySymbol recordTypeName)
{
    if (isTypeDefined(typeEnv, recordTypeName))
        return isTypeRecord(getTypeFromName(typeEnv, recordTypeName));
    return false;
}
    ...
    myFieldRecordCreationExp fieldRecordCreationExp)
{
    ...
    myTypeFieldList typeFields =
        getTypeFromName(typeEnv, recordTypeName)->u.typeRecord->fieldList;
...
//  forward declarations
bool isTwoComparisonOperandsTypeMatchOrNil(
...
//      2.return Integer 1 for true, 0 for false.
//  RETURN:
//      if succeed, it returns a myTranslationAndType variable containing
//          the intermediate code and this expression's result type;
//      if failed, it returns SEMANTIC_ERROR.  
    ...
    myTable varAndFuncEnv, myTable typeEnv, myComparisonExp comparisonExp)
{
    ...
    {
        isOperandsTypeMatches = isTwoComparisonOperandsTypeMatchOrNil(
    ...
    myTable varAndFuncEnv, myTable typeEnv, myAssignmentExp assignmentExp)
{
    myType leftOperandType =
        getLValueExpResultType(varAndFuncEnv, typeEnv, assignmentExp->lValueExp);
    myType rightOperandType =
        getExpResultType(varAndFuncEnv, typeEnv, assignmentExp->exp);
...
//  DO:
//      1.check whether type `type-id2` is already defined.
//      2.define this new type as the same type of `type-id2`.
//  RETURN:
    ...
    myTable typeEnv, mySymbol newTypeName, mySymbol existedTypeName)
{
    ...
    {
        myType existedType = getActualType(typeEnv, existedTypeName);
        MyEnvironment_addNewType(typeEnv, newTypeName, existedType);
        return true;
    }
    else
    {
    ...
    myTable typeEnv, mySymbol newTypeName, myTyFieldList fields)
{
    ...
    {
        MyEnvironment_addNewType(typeEnv, newTypeName, recordType);
        return true;
    }
    else
        return false;
...
//  forwards
myTypeFieldList makeTypeFieldsFromTyFields(myTable typeEnv, myTyFieldList tyFields);
myType makeRecordFromTyFields(myTable typeEnv, myTyFieldList tyFields)
{
    myTypeFieldList typeFields = makeTypeFieldsFromTyFields(typeEnv, tyFields);

    if (typeFields != SEMANTIC_ERROR)
        return makeType_Record(typeFields);
    else
        return SEMANTIC_ERROR;
}
...
myTypeFieldList makeTypeFieldsFromTyFields(myTable typeEnv, myTyFieldList tyFields)
{
    ...
    if (isTypeDefined(typeEnv, oneTyField->typeName))
    {
        myTypeField oneTypeField = makeType_TypeField(
            oneTyField->varName,
            getActualType(typeEnv, oneTyField->typeName));

        myTypeFieldList rests = makeTypeFieldsFromTyFields(typeEnv, tyFields->next);
    ...
    myTable typeEnv, mySymbol newTypeName, mySymbol arrayTypeName)
{
    if (isTypeDefined(typeEnv, arrayTypeName))
    {
        myType elementType = MyEnvironment_getTypeFromName(typeEnv, arrayTypeName);
        myType newArrayType = makeType_Array(elementType);
        MyEnvironment_addNewType(typeEnv, newTypeName, newArrayType);
        return true;
    }
    else
    {
        MyError_pushErrorCodeWithMsg(ERROR__TYPEDEC_ARRAY__TYPE_NOT_DEFINED,
            MySymbol_GetName(arrayTypeName));
        return false;
    }
}
...
//  DO:
//      delegate
bool MySemantic_Dec_Type(myTable varAndFuncEnv, myTable typeEnv, myTypeDec typeDec)
{
    ...
    {
        case NamedType:
            return MySemantic_Dec_Type_Named(
                typeEnv, newTypeName, typeDec->type->u.typeName);
        case RecordType:
            return MySemantic_Dec_Type_Record(typeEnv,
                newTypeName, typeDec->type->u.tyFieldList);
        case ArrayType:
            return MySemantic_Dec_Type_Array(typeEnv,
    ...
    myTable varAndFuncEnv, myTable typeEnv, myShortFormVar shortFormVar)
{
    ...
    {
        myType valueType =
            getExpResultType(varAndFuncEnv, typeEnv, shortFormVar->exp);
    ...
    myTable varAndFuncEnv, myTable typeEnv, myLongFormVar longFormVar)
{
    ...
    if (isTypeMatches)
    {
        myVarAndFuncEntry varEntry =
            myEnvironment_makeVarEntry(getTypeFromName(typeEnv, longFormVar->type));
...
{
    myType valueType = 
        getExpResultType(varAndFuncEnv, typeEnv, longFormVar->exp);
    myType givenType = getTypeFromName(typeEnv, longFormVar->type);
...
void processProcedureErrors(
    bool isParamTypesDefined, bool isBodyLegal,
    bool isBodyNoReturn, mySymbol procedureName);
...
//      2.check whether `exp` is a legal expression.
//      3.check whether `exp` does not return result values.
//  RETURN:
//      a boolean variable indicates whether this function succeeds.
//  STATUS:
//      Tested.
    ...
    myTable varAndFuncEnv, myTable typeEnv, myProcedureDec procedureDec)
{
    ...
    bool isBodyLegal =
        isExpLegal(varAndFuncEnv, typeEnv, procedureDec->exp);
    ...
    if (isParamTypesDefined && isBodyLegal && isBodyNoReturn)
    {
    ...
    {
        processProcedureErrors(isParamTypesDefined, isBodyLegal,
            isBodyNoReturn, procedureDec->name);
        return false;
    }
}
...
bool isFuncParamTypesDefined(myTable typeEnv, myTyFieldList funcFields)
{
    while (funcFields)
    {
        ...
        if (!isTypeDefined(typeEnv, paramField->typeName))
            return false;
    ...
    myTable varAndFuncEnv, myTable typeEnv, myTyFieldList formals)
{
    while (formals)
    {
        ...
        myType formalVarType =
            getTypeFromName(typeEnv, field->typeName);
...
void AddOneProcedure(
    myTable varAndFuncEnv, myTable typeEnv, myProcedureDec procedureDec)
{
    AddOneFunc(varAndFuncEnv, typeEnv, procedureDec->name,
    ...
    mySymbol funcName, myTyFieldList tyFields, myType returnType)
{
    myTypeList formalTypes =
        getTypesFromTyFields(typeEnv, tyFields); 
    ...
    myTable typeEnv, myTyFieldList funcFields)
{
    ...
    myType fieldType = getTypeFromName(
        typeEnv, funcFields->field->typeName);
    return makeType_TypeList(
        fieldType,
        getTypesFromTyFields(typeEnv, funcFields->next));
}
    ...
    bool isBodyNoReturn, mySymbol procedureName)
{
    ...
    if (!isBodyLegal)
        MyError_pushErrorCodeWithMsg(
            ERROR__FUNCDEC_PROCEDURE__BODY_ILLEGAL,
            MySymbol_GetName(procedureName));
    if (!isBodyNoReturn)
        MyError_pushErrorCodeWithMsg(
...
void AddOneFunction(
    myTable varAndFuncEnv, myTable typeEnv, myFunctionDec functionDec);
void processFunctionErrors(
    bool isParamTypesDefined, bool isReturnTypeDefined,
    bool isBodyLegal, bool isReturnTypeMatches, mySymbol functionName);
...
//      3.check whether `exp` is a legal expression.
//      4.check whether the type of `exp` matches type `type-id`(or nil
//          when `type-id` is of type Record).
//  RETURN:
//      a boolean variable indicates whether this function succeeds.
//  STATUS:
//      Tested.
    ...
    myTable varAndFuncEnv, myTable typeEnv, myFunctionDec functionDec)
{
    ...
    {
        processFunctionErrors(
            isParamTypesDefined, isReturnTypeDefined,
            isBodyLegal, isReturnTypeMatches, functionDec->name);
        return false;
    }
} 
...
{
    myType bodyType =
            getExpResultType(varAndFuncEnv, typeEnv, functionDec->exp);
    myType functionReturnType =
            getTypeFromName(typeEnv, functionDec->returnType);
    ...
    myTable varAndFuncEnv, myTable typeEnv, myFunctionDec functionDec)
{
    ...
    AddOneFunc(varAndFuncEnv, typeEnv, functionDec->name,
        functionDec->tyFieldList, returnType);
}
    ...
    bool isBodyLegal, bool isReturnTypeMatches, mySymbol functionName)
{
    ...
    if (!isBodyLegal)
        MyError_pushErrorCodeWithMsg(
            ERROR__FUNCDEC_FUNCTION__BODY_NOT_LEGAL,
            functionNameString);
    ...
    myTable varAndFuncEnv, myTable typeEnv, myFuncDec funcDec)
{
    ...
    {
        case ProcedureDec:
            return MySemantic_Dec_Func_Procedure(
                varAndFuncEnv, typeEnv, funcDec->u.procedureDec);
        case FunctionDec:
            return MySemantic_Dec_Func_Function(
...
//  a delegate function.
bool MySemantic_Dec(myTable varAndFuncEnv, myTable typeEnv, myDec dec)
{
    switch (dec->kind)
    {
        case TypeDec:
            return MySemantic_Dec_Type(varAndFuncEnv, typeEnv, dec->u.tyDec);
            break;
        case VarDec:
            return MySemantic_Dec_Var(varAndFuncEnv, typeEnv, dec->u.varDec);
            break;
        case FuncDec:
            return MySemantic_Dec_Func(varAndFuncEnv, typeEnv, dec->u.funcDec);
            break;
        default:
            assert(false);
    }
    ...
    myTable varAndFuncEnv, myTable typeEnv, myLetExp letExp)
{
    ...
    bool areDeclarationsLegal =
        MySemantic_Decs(varAndFuncEnv, typeEnv, letExp->decList);
    bool isBodyLegal =
    ...
    myTable varAndFuncEnv, myTable typeEnv, myExpList exps)
{
    ...
    while (exps->next)
        exps = exps->next;
    return getExpResultType(varAndFuncEnv, typeEnv, exps->exp);
}
...
//  todo:   Break can only inside while or for loop.
/**
