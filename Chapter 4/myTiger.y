...
dec		: tydec				{ $$ = makeMyDec_Type(getCurrentPosInfo(), $1); }
 ...
 tydec		: TYPE build_in_type EQUAL_SIGN ID			{ $$ = makeMyTyDec(
														MySymbol_MakeSymbol($2),
														makeMyTy_Named(
															getCurrentPosInfo(),
															MySymbol_MakeSymbol($4))); }
		| TYPE ID EQUAL_SIGN ID					{ $$ = makeMyTyDec(
														MySymbol_MakeSymbol($2),
														makeMyTy_Named(
															getCurrentPosInfo(),
															MySymbol_MakeSymbol($4))); }
		| TYPE build_in_type EQUAL_SIGN ty		{ $$ = makeMyTyDec(
														MySymbol_MakeSymbol($2),
														$4); }
		| TYPE ID EQUAL_SIGN ty					{ $$ = makeMyTyDec(
														MySymbol_MakeSymbol($2),
														$4); }
		;
...
ty		: build_in_type									{ $$ = makeMyTy_Named(
																getCurrentPosInfo(),
																MySymbol_MakeSymbol($1)); }
		| LEFT_CURLY_BRACE tyfields RIGHT_CURLY_BRACE	{ $$ = makeMyTy_Record(
																getCurrentPosInfo(),
																$2); }
		| ARRAY OF build_in_type						{ $$ = makeMyTy_Array(
																getCurrentPosInfo(),
																MySymbol_MakeSymbol($3)); }
		| ARRAY OF ID									{ $$ = makeMyTy_Array(
																getCurrentPosInfo(),
																MySymbol_MakeSymbol($3)); }
		;
tyfields	: ID COLON build_in_type tyfields_aux		{ $$ = makeMyTyFieldList(
																makeMyTyField(
																	MySymbol_MakeSymbol($1),
																	MySymbol_MakeSymbol($3)),
																$4); }
		| ID COLON ID tyfields_aux						{ $$ = makeMyTyFieldList(
																makeMyTyField(
																	MySymbol_MakeSymbol($1),
																	MySymbol_MakeSymbol($3)),
																$4); }
|												{ $$ = (myTyFieldList)NULL; }
		;
tyfields_aux	: COMMA ID COLON build_in_type tyfields_aux	{ $$ = makeMyTyFieldList(
																	makeMyTyField(
																		MySymbol_MakeSymbol($2),
																		MySymbol_MakeSymbol($4)),
																	$5); }
		| COMMA ID COLON ID tyfields_aux					{ $$ = makeMyTyFieldList(
																	makeMyTyField(
																		MySymbol_MakeSymbol($2),
																		MySymbol_MakeSymbol($4)),
																	$5); }
...
vardec		: VAR ID ASSIGNMENT_SIGN exp					{ $$ = makeMyVarDec_ShortForm(
																	makeMyShortFormVar(
																		MySymbol_MakeSymbol($2),
																		$4)); }
		| VAR ID COLON build_in_type ASSIGNMENT_SIGN exp	{ $$ = makeMyVarDec_LongForm(
																	makeMyLongFormVar(
																		MySymbol_MakeSymbol($2),
																		MySymbol_MakeSymbol($4),
																		$6)); }
		| VAR ID COLON ID ASSIGNMENT_SIGN exp				{ $$ = makeMyVarDec_LongForm(
																	makeMyLongFormVar(
																		MySymbol_MakeSymbol($2),
																		MySymbol_MakeSymbol($4),
																		$6)); }
...
fundec		: FUNCTION ID LEFT_PARENTHESIS tyfields RIGHT_PARENTHESIS EQUAL_SIGN exp					{ $$ = makeMyFuncDec_Procedure(
																												makeMyProcedureDec(
																													MySymbol_MakeSymbol($2),
																													$4,
																													$7)); }
		| FUNCTION ID LEFT_PARENTHESIS tyfields RIGHT_PARENTHESIS COLON build_in_type EQUAL_SIGN exp	{ $$ = makeMyFuncDec_Function(
																												makeMyFunctionDec(
																													MySymbol_MakeSymbol($2),
																													$4,
																													MySymbol_MakeSymbol($7),
																													$9)); }
		| FUNCTION ID LEFT_PARENTHESIS tyfields RIGHT_PARENTHESIS COLON ID EQUAL_SIGN exp				{ $$ = makeMyFuncDec_Function(
																												makeMyFunctionDec(
																													MySymbol_MakeSymbol($2),
																													$4,
																													MySymbol_MakeSymbol($7),
...
l_value		: ID l_value_aux							{
														  $$ = makeMyLValue(
																...
																$2); }
     		;
l_value_aux	: DOT ID l_value_aux						{ $$ = makeMyLValueAux(
																MySymbol_MakeSymbol($2),
																(myExp)NULL,
																$3); }
		| LEFT_BRACKET exp RIGHT_BRACKET l_value_aux	{ $$ = makeMyLValueAux(
																(mySymbol)NULL,
																$2,
																$4); }
     		|											{ $$ = (myLValueAux)NULL; }
...
function_call	: ID LEFT_PARENTHESIS RIGHT_PARENTHESIS					{ $$ = makeMyFunctionCallExp_NoParam(
																				makeMyNoParamFunctionCallExp(
																					MySymbol_MakeSymbol($1))); }
		| ID LEFT_PARENTHESIS exp function_call_aux RIGHT_PARENTHESIS	{ $$ = makeMyFunctionCallExp_Param(
																				makeMyParamFunctionCallExp(
																					MySymbol_MakeSymbol($1),
																					makeMyExpList(
																						$3, $4))); }
		;
function_call_aux	: COMMA exp function_call_aux						{ $$ = makeMyExpList($2, $3); }
			|															{ $$ = (myExpList)NULL; }
			;
...
array_creation	: build_in_type LEFT_BRACKET exp RIGHT_BRACKET OF exp	{ $$ = makeMyArrayCreationExp(
																				MySymbol_MakeSymbol($1),
																				$3,
																				$6); }
		| ID LEFT_BRACKET exp RIGHT_BRACKET OF exp						{ $$ = makeMyArrayCreationExp(
																				MySymbol_MakeSymbol($1),
																				$3,
																				$6); }
		;

record_creation	: build_in_type LEFT_CURLY_BRACE ID EQUAL_SIGN exp record_creation_aux RIGHT_CURLY_BRACE	{ $$ = makeMyRecordCreationExp_Field(
																													makeMyFieldRecordCreationExp(
																														...
																														makeMyRecordFieldList(
																															makeMyRecordField(
																																...
																																$5),
																															$6))); }
		| ID LEFT_CURLY_BRACE ID EQUAL_SIGN exp record_creation_aux RIGHT_CURLY_BRACE						{ $$ = makeMyRecordCreationExp_Field(
																													makeMyFieldRecordCreationExp(
																														MySymbol_MakeSymbol($1),
																														makeMyRecordFieldList(
																															makeMyRecordField(
																																...
																																$5),
																															$6))); }
		| build_in_type LEFT_CURLY_BRACE RIGHT_CURLY_BRACE													{ $$ = makeMyRecordCreationExp_NoField(
																													makeMyNoFieldRecordCreationExp(
																														MySymbol_MakeSymbol($1))); }
		| ID LEFT_CURLY_BRACE RIGHT_CURLY_BRACE																{ $$ = makeMyRecordCreationExp_NoField(
																													makeMyNoFieldRecordCreationExp(
																														MySymbol_MakeSymbol($1))); }
		;
record_creation_aux	: COMMA ID EQUAL_SIGN exp record_creation_aux	{ $$ = makeMyRecordFieldList(
																			makeMyRecordField(
																				MySymbol_MakeSymbol($2),
																				$4),
																			$5); }
...
parentheses	: LEFT_PARENTHESIS exp RIGHT_PARENTHESIS	{ $$ = makeMyParenthesesExp($2); }
		;

no_value	: LEFT_PARENTHESIS  RIGHT_PARENTHESIS		{ $$ = makeMyNoValueExp(); }
		;

sequencing	: LEFT_PARENTHESIS exp SEMICOLON exp sequencing_aux RIGHT_PARENTHESIS	{ $$ = makeMySequencingExp(
																							$2,
																							$4,
																							$5); }
		;
sequencing_aux	: SEMICOLON exp sequencing_aux										{ $$ = makeMyExpList(
																							$2,
																							$3); }
		|																			{ $$ = (myExpList)NULL; }
		;

for		: FOR ID ASSIGNMENT_SIGN exp TO exp DO exp		{ $$ = makeMyForExp(
																MySymbol_MakeSymbol($2),
																$4,
																$6,
																$8); }
...
if_then_else	: IF exp THEN exp ELSE exp				{ $$ = makeMyIfThenElseExp(
																$2,
																$4,
																$6); }
		;

if_then		: IF exp THEN exp							{ $$ = makeMyIfThenExp(
																$2,
																$4); }
		;

comparison	: exp EQUAL_SIGN exp						{ $$ = makeMyComparisonExp_Equal($1, $3); }
		| exp NOT_EQUAL_SIGN exp						{ $$ = makeMyComparisonExp_NotEqual($1, $3); }
		| exp LESS_SIGN exp								{ $$ = makeMyComparisonExp_SmallerThan($1, $3); }
		| exp LESS_EQUAL_SIGN exp						{ $$ = makeMyComparisonExp_SmallerEqualThan($1, $3); }
		| exp GREATER_SIGN exp							{ $$ = makeMyComparisonExp_LargerThan($1, $3); }
		| exp GREATER_EQUAL_SIGN exp					{ $$ = makeMyComparisonExp_LargerEqualThan($1, $3); }
		;

boolean_operate	: exp LOGICAL_AND exp					{ $$ = makeMyBooleanOperateExp_And($1, $3); }
		| exp LOGICAL_OR exp							{ $$ = makeMyBooleanOperateExp_Or($1, $3); }
		;

assignment	: l_value ASSIGNMENT_SIGN exp				{ $$ = makeMyAssignmentExp($1, $3); }
		;
...
negation	: MINUS_SIGN exp %prec DIVIDE_SIGN			{ $$ = makeMyNegationExp($2); }
		;

break		: BREAK										{ $$ = makeMyBreakExp(); }
		;

exp		: l_value			{ $$ = makeMyExp_LValue(getCurrentPosInfo(), $1); }
		| function_call		{ $$ = makeMyExp_FunctionCall(getCurrentPosInfo(), $1); }
		| nil				{ $$ = makeMyExp_Nil(getCurrentPosInfo(), $1); }
		| integer_literal	{ $$ = makeMyExp_IntegerLiteral(getCurrentPosInfo(), $1); }
		| string_literal	{ $$ = makeMyExp_StringLiteral(getCurrentPosInfo(), $1); }
		| array_creation	{ $$ = makeMyExp_ArrayCreation(getCurrentPosInfo(), $1); }
		| record_creation	{ $$ = makeMyExp_RecordCreation(getCurrentPosInfo(), $1); }
		| arithmetic		{ $$ = makeMyExp_Arithmetic(getCurrentPosInfo(), $1); }
		| parentheses		{ $$ = makeMyExp_Parentheses(getCurrentPosInfo(), $1); }
		| no_value			{ $$ = makeMyExp_NoValue(getCurrentPosInfo(), $1); }
		| sequencing		{ $$ = makeMyExp_Sequencing(getCurrentPosInfo(), $1); }
		| for				{ $$ = makeMyExp_For(getCurrentPosInfo(), $1); }
		| if_then_else		{ $$ = makeMyExp_IfThenElse(getCurrentPosInfo(), $1); }
		| if_then			{ $$ = makeMyExp_IfThen(getCurrentPosInfo(), $1); }
		| comparison		{ $$ = makeMyExp_Comparison(getCurrentPosInfo(), $1); }
		| boolean_operate	{ $$ = makeMyExp_BooleanOperate(getCurrentPosInfo(), $1); }
		| assignment		{ $$ = makeMyExp_Assignment(getCurrentPosInfo(), $1); }
		| let				{ $$ = makeMyExp_Let(getCurrentPosInfo(), $1); }
		| while				{ $$ = makeMyExp_While(getCurrentPosInfo(), $1); }
		| negation			{ $$ = makeMyExp_Negation(getCurrentPosInfo(), $1); }
		| break				{ $$ = makeMyExp_Break(getCurrentPosInfo(), $1); }
		;
