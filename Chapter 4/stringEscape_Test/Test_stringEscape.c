...
void translateEscape_ShouldWorkOnSingleEscapeCharString(void)
{
    ...
    //	verify
    CU_ASSERT(strlen(actual_newline) == 1 &&
		    actual_newline[0] == '\x0A');
    CU_ASSERT(strlen(actual_tab) == 1 &&
		    actual_tab[0] == '\x09');
    CU_ASSERT(strlen(actual_double_quote) == 1 &&
		    actual_double_quote[0] == '\x22');
    CU_ASSERT(strlen(actual_backslash) == 1 &&
		    actual_backslash[0] == '\x5C');
}
