...
{
    if (invalidSymbol(varName) || invalidSymbol(typeName))
...
{
    if (invalidSymbol(typeName))	return (myTy)NULL;
...
{
    if (invalidSymbol(arrayTypeName))	return (myTy)NULL;
...
{
    if (invalidSymbol(name))	return (myTyDec)NULL;
...
{
    if (invalidSymbol(name) || invalidSymbol(type))
...
{
    if (invalidSymbol(name))	return (myShortFormVar)NULL;
...
{
    if (invalidSymbol(name))	return (myProcedureDec)NULL;
...
{
    if (invalidSymbol(name) || invalidSymbol(returnType))
...
{
    if (invalidSymbol(id))	return (myRecordFieldAux)NULL;
...
/*	To test, uncomment 'static'	*/	
static enum myLValueKind getLValueKind(myLValueAux aux)
{
...
myLValueExp makeMyLValue(myPos pos, mySymbol id, myLValueAux aux)
{
    ...
    if (invalidSymbol(id))	return (myLValueExp)NULL;
...
{
    if (invalidSymbol(name))	return NULL;
...
{
    if (invalidSymbol(varName))	return (myRecordField)NULL;
